...
const addProduct=async(req,res)=>{
    try {
        ...
        if(!firm){
            return res.status(402).json({message:'firm not found'})
        }
       ...
       res.status(200).json(savedProduct)
        ...
        console.log(error)
        return res.status(500).json({message:'internal error in add item'})
    }
}
...
const getProductByFirm=async(req,res)=>{
    try {
        ...
        const resortName=firm.firmName
        const products=await Product.find({firm:firmId})
       res.status(200).json({resortName,products})
